...
parser$add_argument("--ylab1", required=F, default="NA", type="character",
                    help="Specify the y-axis title for GWAS1. Default=-log10(P).")
parser$add_argument("--ylab2", required=F, default="NA", type="character",
                    help="Specify the y-axis title for GWAS2. Default=-log10(P).")
parser$add_argument("--save-rds", dest="save_rds", action="store_true", default=F,
...
#######
# trait.gwas1 <- "eQTL"
# file.gwas1 <- "/data1/sanghyeon/Projects/mr_drug_repurposing/src/eQTL/v2/GTEx_actionable/actionable.GTEx_Adrenal_Gland.txt.gz"
# delim.gwas1 <- "whitespace"
# snp.gwas1 <- "rsid"
# chr.gwas1 <- "chr_b37"
# pos.gwas1 <- "pos_b37"
# p.gwas1 <- "P-value"
# 
# trait.gwas2 <- "SSc GWAS"
# file.gwas2 <- "/data1/soohyun/Systemic_sclerois/data_SSC/reformat.IA.GWAS.BakkerMK.2020.sumstats.Stage_1_clean.txt"
# delim.gwas2 <- "tab"
# snp.gwas2 <- "SNP"
# chr.gwas2 <- "CHR"
# pos.gwas2 <- "BP"
# p.gwas2 <- "P"
# 
# file.geneloc <- "gene.loc"
# delim.geneloc <- "tab"
# gene.geneloc <- "gene_symbol"
# chr.geneloc <- "chr"
# start.geneloc <- "start"
# end.geneloc <- "end"
# 
...
for (idx in 1:nrow(df.geneloc)){
    ...
    if (extract_gene_gwas2){
        ...
        df.gwas2.sub <- df.gwas2 %>%
            filter(GENE.extract == gene_to_extract) %>%
            dplyr::filter(chrom == chr & pos >= (start_pos - flanking_window*1000) & pos <= (end_pos + flanking_window*1000)) %>%
            dplyr::select(chrom, pos, rsid, p)
    } else{
        df.gwas2.sub <- df.gwas2 %>%
    ...
    if (require(EnsDb.Hsapiens.v75)){
        # loc1 <- locus(data=df.gwas1.sub, gene = gene, flank = (flanking_window*1000),
        #                   ens_db = "EnsDb.Hsapiens.v75", LD = "R2", index_snp=snp1)
        loc1 <- locus(data=df.gwas1.sub,
                      seqname=chr,
                      xrange=c((start_pos - flanking_window*1000), (end_pos + flanking_window*1000)),
                      ens_db="EnsDb.Hsapiens.v75",
                      LD="R2",
                      index_snp=snp1)
    }
    if (require(EnsDb.Hsapiens.v75)){
        # loc2 <- locus(data=df.gwas2.sub, gene = gene, flank = (flanking_window*1000),
        #               ens_db = "EnsDb.Hsapiens.v75", LD = "R2", index_snp=snp2)
