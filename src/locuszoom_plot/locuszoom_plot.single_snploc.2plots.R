...
# SNPLOC file
parser$add_argument("--snp-snploc", dest="snp.snploc", required=T,
                    help="Specify SNP of snploc.")
parser$add_argument("--chr-snploc", dest="chr.snploc", required=T,
                    help="Specify CHR of snploc.")
parser$add_argument("--pos-snploc", dest="pos.snploc", required=T, type = "integer",
                    help="Specify POS of snploc.")
...
snp1 <- snp.snploc
chr <- as.character(chr.snploc)
snp.pos <- pos.snploc

## Subset GWAS1 and compute R2 between selected SNP
df.gwas1.sub <- df.gwas1 %>%
    dplyr::filter(chrom == chr & pos >= (snp.pos - flanking_window*1000) & pos <= (snp.pos + flanking_window*1000)) %>%
...
if (require(EnsDb.Hsapiens.v75)){
    loc1 <- locus(data=df.gwas1.sub,
                    seqname=chr,
                    xrange=c((snp.pos - flanking_window*1000), (snp.pos + flanking_window*1000)),
