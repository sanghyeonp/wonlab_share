...
#   - Drop: all_meta_AF < 0.01
# Muli-allelic (flag)
#   - Drop: all multi-allelic
# INDEL (flag)
#   - Drop: all INDEL
# SNPs with NA
#   - NA for CHR, POS, REF, ALT, AF, BETA, SE
# SNPs with invalid values
#   - AF < 0 OR AF > 1
#   - SE <= 0 OR SE == Inf
#   - abs(BETA) == Inf
#   - Alleles that are not A, C, G, or T
# Autosome (flag)
# Monomorphic SNP
#  - AF == 0 OR AF == 1

### <<< DELETE
file.gwas <- "/data1/sanghyeon/Projects/atherosclerosis_gwas/data/gwas/AAA/AAA_Bothsex_eur_inv_var_meta_GBMI_052021_nbbkgt1.txt.gz"
df.gwas <- fread(file.gwas, sep="\t", data.table=F, nThread=30)

# df_gwas <- df.gwas[sample(1:nrow(df.gwas), 10000),]

df_gwas=df.gwas
snp_col="rsid" # Default=NA
chr_col="#CHR" # Default=NA
pos_col="POS" # Default=NA
a1_col="REF" # Default=NA
a2_col="ALT" # Default=NA
af_col="all_meta_AF" # Default=NA
effect_col="inv_var_meta_beta" # Default=NA
se_col="inv_var_meta_sebeta" # Default=NA
info_col=NA # Default=NA

threshold.AF=0.01 # Default=0.01
threshold.INFO=0.9 # Default=0.9

prefix="test_v2" # Default=""

save.snp_count="csv" # Default=NA, Options=c("txt", "tsv", "csv". "rds")
save.snp_list="rds" # Default=NA, Options=c("txt", "tsv", "csv". "rds")

additional_columns="inv_var_het_p;is_diff_AF_gnomAD" # Default=NA, Format="column;column;column;..."
additional_criteria="inv_var_het_p<=0.05;is_diff_AF_gnomAD!='no'" # Default=NA, Format="criterion;criterion;criterion;..."
"
SNP이 없으면, chr, pos 라도 있어야함. (for multi-allelic check)
"
...
###
# basic_GWAS_filter_criteria <- function()
