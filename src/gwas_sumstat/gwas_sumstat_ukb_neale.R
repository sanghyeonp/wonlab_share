...
print("::Run:: Extract CHR, POS, REF, ALT and EAF")
df <- df %>%
...
### Map rsID
print("::Run:: Mapping rsID")
df <- merge(df, df_variant, by='variant')
nsnp <- nrow(df)
print(paste0("\tNumber of SNPs before: ", nsnp))
df2 <- df[grepl("^rs", df$RSID), ]
print(paste0("\tNumber of SNPs without rsID: ", nsnp - nrow(df2)))
if (keep_no_rsid == FALSE){
    df <- df2
    rm(df2)
    print(paste0("\tNumber of SNPs after filtering unmapped rsID: ", nrow(df)))
}

### MAF filter
if (maf_filter != 0){
    print(paste0("::Run:: MAF filter with threshold: ", maf_filter))
    print(paste0("\tNumber of SNPs before: ", nrow(df)))
    df <- df[df$MAF > maf_filter, ]
    print(paste0("\tNumber of SNPs after: ", nrow(df)))
} else{
    print(paste0("::Run:: No MAF filtering"))
}

### Remove duplicates
print("::Run:: Handle duplicated SNP")
df2 <- df %>%
    distinct(RSID, .keep_all = TRUE)
print(paste0("\tNumber of SNPs before: ", nrow(df)))
print(paste0("\tNumber of duplicated SNPs: ", nrow(df) - nrow(df2)))
...
### Remove rows with NA
print("::Run:: Handle rows with NA")
df2 <- df[complete.cases(df), ]
print(paste0("\rNumber of rows: ", nrow(df)))
print(paste0("\rNumber of rows with NA: ", nrow(df) - nrow(df2)))
if (keep_na == FALSE){
    print("\tRemoving rows with NA...")
    df <- df2
    rm(df2)
    print(paste0("\tNumber of rows after: ", nrow(df)))
} else{
    print("\tKeeping rows with NA...")
}


### Subset specified columns
print("::Run:: Subset specified columns")
print(paste0("\tColumns specified: ", paste(retain_col, collapse=", ")))

df <- subset(df, select=as.vector(retain_col))


### Make column name renaming vector
print("::Run:: Rename the columns")
if (rename_col[1] == "NA"){
    print("\t`--rename-col` not specified. No renaming done.")
} else{
...
### Save the output
print("::Run:: Save output GWAS summary statistics")
if (outf == "NA"){
...
print(paste0("\tSaved at: ", out_path))
