...
reference_panel_list <- c("1kG" = "/data1/sanghyeon/wonlab_contribute/combined/data_common/reference_panel/1kGp3/EUR/CHR/reference.1kG.EUR.maf_0.005.geno_0.02.chr",
                          ...
                          "KoGES_random_10k"="/data1/sanghyeon/wonlab_contribute/combined/data_common/reference_panel/KoGES/CHR/KoGES_random10k.rsid.b37.drop_dup_chr"
)
  ...
  if (nrow(df_gwas1_snplist.chr) != 0){
    ...
    #                           distinct(secondarySNP, .keep_all = TRUE) # Important note 2
      ...
      if(nrow(df_gwas2_snplist.chr.filter ) != 0){
        other_snplist <- df_gwas2_snplist.chr.filter$SNP
        ...
        df_tmp <- df_gwas2_snplist.chr.filter  %>%
                    dplyr::select(SNP, P) %>%
                    filter(SNP %in% snp_gwas2_tested)
        df_tmp <- merge(df_tmp, plink_ld_result %>% dplyr::select(-ref_snp), by.x = "SNP", by.y = "other_snp", all.x = T)
        ...
        #   if (ref_snp %in% snp_gwas2_tested[idx_in_ld]){
        #     snp_gwas2_in_ld_index_snp <- ref_snp
        #     index_snp_idx <- which(snp_gwas2_tested[idx_in_ld] == ref_snp)

        #   }
        #   snp_gwas2_in_ld_index_snp <- "NA" # 여기에 R2가 높은거에 대한 대표 SNP 선정필요
        }
