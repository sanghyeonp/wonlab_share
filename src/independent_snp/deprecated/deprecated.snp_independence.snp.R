...
parser$add_argument("--snp-col-gwas2-snplist", dest="snp_gwas2_snplist",  required=TRUE,
                    help="Specify SNP column name.")
parser$add_argument("--chr-col-gwas2-snplist", dest="chr_gwas2_snplist",  required=TRUE,
                    help="Specify chromosome column name.")
parser$add_argument("--pos-col-gwas2-snplist", dest="pos_gwas2_snplist",  required=TRUE,
                    help="Specify position column name.")
parser$add_argument("--pval-col-gwas2-snplist", dest="pval_gwas2_snplist",  required=TRUE,
...
chr_gwas2_snplist <- args$chr_gwas2_snplist
pos_gwas2_snplist <- args$pos_gwas2_snplist
pval_gwas2_snplist <- args$pval_gwas2_snplist
...
for (chr in 1:22){
  ...
  df_gwas2_snplist.chr <- filter(df_gwas2_snplist, !!as.name(chr_gwas2_snplist) == chr)
  ...
  if (nrow(df_gwas1_snplist.chr) != 0){
    ...
    for (idx in 1:nrow(df_gwas1_snplist.chr)){
      ...
      df_gwas2_snplist.chr.filter <- filter(df_gwas2_snplist.chr, 
                                                  (!!as.name(pos_gwas2_snplist) >= ref_snp.pos - (window * 1000)) &
                                                  (!!as.name(pos_gwas2_snplist) < ref_snp.pos + (window * 1000)),
                                                  (!!as.name(pval_gwas2_snplist) < p_threshold))

      df_gwas2_snplist.chr.filter  <- df_gwas2_snplist.chr.filter  %>%
                              distinct(!!as.name(snp_gwas2_snplist), .keep_all = TRUE) # Important note 2

      if(nrow(df_gwas2_snplist.chr.filter ) != 0){
        other_snplist <- df_gwas2_snplist.chr.filter[[snp_gwas2_snplist]]
        ...
        df_tmp <- df_gwas2_snplist.chr.filter  %>%
                    dplyr::select(!!as.name(snp_gwas2_snplist), !!as.name(pval_gwas2_snplist)) %>%
                    filter(!!as.name(snp_gwas2_snplist) %in% snp_gwas2_tested)
        df_tmp[[snp_gwas2_snplist]] <- factor(df_tmp[[snp_gwas2_snplist]], levels = snp_gwas2_tested)
        snp_gwas2_tested_pval <- df_tmp[[pval_gwas2_snplist]]
